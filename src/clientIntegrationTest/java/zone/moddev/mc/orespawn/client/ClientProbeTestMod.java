...
public final class ClientProbeTestMod {
	...
	public static void onScreenInitialized(GuiScreenEvent.InitGuiEvent.Post event) {
		...
		for (Widget button : event.getWidgetList()) {
			if (isWorldSettingsButton(button)) probe.worldSettingsButton = button;
		}
	}
	...
	public static void onScreenDrawn(GuiScreenEvent.DrawScreenEvent.Post event) {
		...
		if (probe != null && Boolean.getBoolean("clientprobe.enabled")
				&& isOreSpawnEditor(event.getGui())) probe.editorFrames++;
	}
			...
			switch (state) {
				...
				case 1:
					if (worldSettingsButton == null && minecraft.screen instanceof CreateWorldScreen) {
						for (Widget candidate : widgets(minecraft.screen)) {
					...
					if (minecraft.screen instanceof CreateWorldScreen && worldSettingsButton != null) {
						// Forge 36 invokes the target-native OreSpawn button callback directly.
	...
	private Button nextNavigationButton(OreSpawnWorldSettingsScreen root) {
		for (Widget widget : widgets(root)) {
			if (!(widget instanceof Button) || widget instanceof CycleButton) continue;
			Button button = (Button) widget;
			String caption = TextFormatting.stripFormatting(button.getMessage().getString());
	...
	private static void initializeScreen(Screen screen, Minecraft minecraft) {
		...
		throw new IllegalStateException("Could not locate Forge 36 Screen initialization method");
	}

	private static void pressDone(Screen screen) {
		for (Widget widget : widgets(screen)) {
			if (!(widget instanceof Button)) continue;
			String caption = TextFormatting.stripFormatting(((Button) widget).getMessage().getString());
	...
	private static boolean isWorldSettingsButton(Widget widget) {
		return widget instanceof Button
				&& TextFormatting.stripFormatting(widget.getMessage().getString())
	...
	private static void createWorld(Minecraft minecraft) {
		...
		WorldSettings settings = new WorldSettings("OreSpawn Client Smoke", GameType.CREATIVE,
	...
	private static java.util.List<Widget> widgets(Screen screen) {
		java.util.List<Widget> result = new java.util.ArrayList<>();
		for (net.minecraft.client.gui.IGuiEventListener child : screen.children()) {
			if (child instanceof Widget) result.add((Widget) child);
		}
		return result;
	}

	private static void stopIntegratedServer(Minecraft minecraft) {
		// Match Forge 36's target-native disconnect path. unloadWorld(Screen) clears
	...
	private void writeMarker() throws IOException {
		...
		try (FileOutputStream output = new FileOutputStream(new File("client-smoke-pass.properties"))) {
			values.store(output, "OreSpawn Forge 1.16.5 client integration gate");
		}
	}
