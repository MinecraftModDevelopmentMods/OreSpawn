...
public final class ClientProbeTestMod {
	...
	public static void onScreenInitialized(GuiScreenEvent.InitGuiEvent.Post event) {
		...
		for (GuiEventListener child : event.getWidgetList()) {
			...
			switch (state) {
				...
				case 1:
					...
					if (minecraft.screen instanceof CreateWorldScreen && worldSettingsButton != null) {
						// Forge 37 invokes the target-native OreSpawn button callback directly.
	...
	private static void initializeScreen(Screen screen, Minecraft minecraft) {
		...
		throw new IllegalStateException("Could not locate Forge 37 Screen initialization method");
	}
	...
	private static void createWorld(Minecraft minecraft) {
		...
		WorldGenSettings generator = WorldGenSettings.makeDefault(
				registries.ownedRegistryOrThrow(Registry.DIMENSION_TYPE_REGISTRY),
				registries.ownedRegistryOrThrow(Registry.BIOME_REGISTRY),
				registries.ownedRegistryOrThrow(Registry.NOISE_GENERATOR_SETTINGS_REGISTRY))
	...
	private static void stopIntegratedServer(Minecraft minecraft) {
		// Match Forge 37's target-native disconnect path. clearLevel(Screen) clears
	...
	private void writeMarker() throws IOException {
		...
		try (FileOutputStream output = new FileOutputStream(new File("client-smoke-pass.properties"))) {
			values.store(output, "OreSpawn Forge 1.17.1 client integration gate");
		}
	}
