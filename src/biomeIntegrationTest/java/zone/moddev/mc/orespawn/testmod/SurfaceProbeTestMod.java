...
public final class SurfaceProbeTestMod {
	...
	private void setup(FMLCommonSetupEvent event) {
		event.enqueueWork(() -> {
			BiomeDictionary.addTypes(RegistryKey.create(Registry.BIOME_REGISTRY, BIOME_A),
	...
	private void enqueueProvider(InterModEnqueueEvent event) {
		...
		provider.ore(DYNAMIC_ORE, blockId(Blocks.DIAMOND_BLOCK), ore -> ore
				...
				.dimension(OPEN_ID, placement -> placement
						...
						.hostBlock(blockId(Blocks.GRANITE))
						.biome(BIOME_A)
	...
	private static void addDynamicBiomeGeology(WorldgenProvider.Builder provider) {
		...
		provider.rock(new ResourceLocation(MODID + ":rock/fallback"), blockId(Blocks.ANDESITE),
				GeologyFamily.SEDIMENTARY, rock -> {
					rock.dimensions(java.util.Collections.singleton(OPEN_ID));
					rock.geomeWeight(PROBE_GEOME, 0.0D);
					rock.geomeWeight(PROBE_GEOME_ALTERNATIVE, 0.0D);
					...
					for (int localX = 0; localX < 16; localX++) {
						...
						if (!roofed) {
							for (int depth = 6; depth <= 8; depth++) {
								...
								if (geologyState.is(Blocks.GRANITE)) {
									dictionaryAlternative++;
								} else if (geologyState.is(Blocks.DIORITE)) {
									dictionaryPrimary++;
								} else {
	...
	private static long auditDynamicBiomeOre(ServerWorld level) {
		BlockPos.Mutable pos = new BlockPos.Mutable();
		long count = 0L;
		for (int chunkZ = MINIMUM_CHUNK; chunkZ <= MAXIMUM_CHUNK; chunkZ++) {
			for (int chunkX = MINIMUM_CHUNK; chunkX <= MAXIMUM_CHUNK; chunkX++) {
	...
	private static NaturalSourceAudit auditNaturalSources(ServerWorld level, IChunk chunk,
		...
		for (int index = 0; index < NATURAL_SOURCES.length; index++) {
			...
			if (converted.is(Blocks.DIORITE) || converted.is(Blocks.GRANITE)) rawConverted++;
		...
		if (chunk.getBlockState(pos.set(minX + 11, bedrockGroundY - 24, minZ + 12))
				.is(Blocks.BEDROCK)) {
			bedrockPreserved++;
		}
		...
		if (chunk.getBlockState(pos).is(Blocks.CHEST)
				&& level.getBlockEntity(pos) instanceof ChestTileEntity) {
			ChestTileEntity chest = (ChestTileEntity) level.getBlockEntity(pos);
			if (chest != null && chest.getItem(0).getItem() == Items.EMERALD
	...
	private static void placeRawNaturalSources(ISeedReader world, IChunk chunk,
			BlockPos.Mutable pos, int minX, int minZ) {
		for (int index = 0; index < NATURAL_SOURCES.length; index++) {
			int x = naturalX(minX, index);
			int z = naturalZ(minZ, index);
		...
		if (world.getBlockEntity(pos) instanceof ChestTileEntity) {
			...
			sentinel.setHoverName(new StringTextComponent(RAW_CHEST_ITEM_NAME));
	...
	private static void placeWeatherMaterialSentinels(ISeedReader world, IChunk chunk,
			BlockPos.Mutable pos, int minX, int minZ) {
	...
	private static void placeAuthoredNaturalSources(ISeedReader world, IChunk chunk,
	...
	private static final class AuditResult {
		...
		private final long dictionaryPrimary;
		private final long dictionaryAlternative;
		private final long dynamicBiomeOre;
		private final long exposedSnowConverted;
		private final long surfaceIceConverted;
		private final long buriedSnowPreserved;
		private final long buriedIcePreserved;
		...
		AuditResult(long top, long underwater, long filler, long geology, long ceiling,
				...
				long rawBedrock, long rawBlockEntities, long dictionaryPrimary,
				long dictionaryAlternative, long dynamicBiomeOre,
			...
			this.dictionaryPrimary = dictionaryPrimary;
			this.dictionaryAlternative = dictionaryAlternative;
			this.dynamicBiomeOre = dynamicBiomeOre;
			this.exposedSnowConverted = exposedSnowConverted;
			this.surfaceIceConverted = surfaceIceConverted;
			this.buriedSnowPreserved = buriedSnowPreserved;
			this.buriedIcePreserved = buriedIcePreserved;
		...
		long dictionaryPrimary() { return dictionaryPrimary; }
		long dictionaryAlternative() { return dictionaryAlternative; }
		long dynamicBiomeOre() { return dynamicBiomeOre; }
		long exposedSnowConverted() { return exposedSnowConverted; }
		long surfaceIceConverted() { return surfaceIceConverted; }
		long buriedSnowPreserved() { return buriedSnowPreserved; }
		long buriedIcePreserved() { return buriedIcePreserved; }
