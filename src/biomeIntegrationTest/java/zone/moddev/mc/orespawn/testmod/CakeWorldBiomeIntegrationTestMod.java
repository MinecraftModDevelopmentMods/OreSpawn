...
public final class CakeWorldBiomeIntegrationTestMod {
	...
	private static final ResourceLocation DIMENSION = ResourceLocation.parse("minecraft:the_nether");
	private static final ResourceLocation BIOME = ResourceLocation.parse(MODID + ":cake_plains");
	private static final ResourceLocation PINK_CONCRETE = ResourceLocation.parse("minecraft:pink_concrete");
	private static final ResourceLocation WHITE_CONCRETE = ResourceLocation.parse("minecraft:white_concrete");
	private static final TagKey<Biome> CAKE_BIOMES = TagKey.create(Registries.BIOME,
			ResourceLocation.parse(MODID + ":cake_biomes"));
	...
	private void enqueueProvider(InterModEnqueueEvent event) {
		...
		WorldgenProvider provider = WorldgenProvider.builder(MODID, 1)
				.biomePalette(ResourceLocation.parse(MODID + ":normal_terrain"), DIMENSION,
						palette -> palette
	...
	private static AuditResult auditChunks(ServerLevel level) {
		...
		for (int chunkZ = MINIMUM_CHUNK; chunkZ <= MAXIMUM_CHUNK; chunkZ++) {
			for (int chunkX = MINIMUM_CHUNK; chunkX <= MAXIMUM_CHUNK; chunkX++) {
				...
				ResourceLocation actual = biome.unwrapKey().map(key -> key.location()).orElse(null);
				if (!BIOME.equals(actual)) {
					throw new IllegalStateException("Expected " + BIOME + " at chunk "
							+ chunkX + "," + chunkZ + " but found " + actual);
				}
				if (!biome.is(CAKE_BIOMES)) {
					throw new IllegalStateException("Custom biome is missing its datapack tag: " + BIOME);
				}
				matchingChunks++;
				...
				for (int localZ = 0; localZ < 16; localZ++) {
					for (int localX = 0; localX < 16; localX++) {
						int surfaceY = chunk.getHeight(
								Heightmap.Types.WORLD_SURFACE, localX, localZ) - 1;
		...
		long expectedSurface = (long) expectedChunks * 16L * 16L;
		long expectedFiller = expectedSurface * 3L;
		if (matchingChunks != expectedChunks || pinkSurface != expectedSurface
				|| whiteFiller != expectedFiller) {
			throw new IllegalStateException("Incomplete custom-biome generation: chunks="
					+ matchingChunks + "/" + expectedChunks + ", pink=" + pinkSurface + "/"
					+ expectedSurface + ", white=" + whiteFiller + "/" + expectedFiller);
		}
