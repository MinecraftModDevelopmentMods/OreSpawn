...
public final class BakedGeomeConfig {
	...
	BakedGeomeConfig(GeomeDefinition[] geomes, double geomeScale, double biomeInfluence,
			...
			Map<ResourceLocation, double[]> biomeWeightsById, RockEntry[] rocks, FormationSettings formations) {
		this.geomes = geomes;
	...
	int scoreGeomes(Biome biome, ResourceLocation biomeId, double[] regionalNoiseAndScores, double boundaryNoise) {
		double[] weights = biomeWeightsFor(biome, biomeId);
		double bestScore = Double.NEGATIVE_INFINITY;
		int bestIndex = 0;
		for (int i = 0; i < geomes.length; i++) {
	...
	RockFamily pickStableFamilyAtWorldY(int geomeIndex, int worldY, int formationY,
		...
		if (hasEligibleStableRock(geomeIndex, preferred, worldY, formationY)) return preferred;
		...
		for (RockFamily family : RockFamily.values()) {
			if (!hasEligibleStableRock(geomeIndex, family, worldY, formationY)) continue;
	...
	IBlockState pickStableRockAtWorldY(int geomeIndex, RockFamily family, int worldY,
		...
		for (int rockIndex : familyRockIndexes[family.ordinal()]) {
			if (!isEligibleStableRock(geomeIndex, rockIndex, worldY, yIndex)) continue;
			...
			for (int y = MIN_Y; y <= MAX_Y; y++) {
				...
				for (int bucket = 0; bucket < FORMATION_BUCKETS; bucket++) {
					...
					for (RockFamily family : RockFamily.values()) {
						...
						for (int rockIndex : familyRockIndexes[familyIndex]) {
							RockEntry rock = rocks[rockIndex];
							if (y < rock.minY || y > rock.maxY) continue;
	...
	private boolean hasEligibleStableRock(int geomeIndex, RockFamily family, int worldY, int formationY) {
		int yIndex = clampStableY(formationY);
		for (int rockIndex : familyRockIndexes[family.ordinal()]) {
			if (isEligibleStableRock(geomeIndex, rockIndex, worldY, yIndex)) return true;
		}
		return false;
	}
