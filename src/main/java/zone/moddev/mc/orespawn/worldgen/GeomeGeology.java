...
public final class GeomeGeology {
		...
		for (int dx = 0; dx < 16; dx++) {
			...
			for (int dz = 0; dz < 16; dz++) {
				...
				if (stableLayers) {
					...
					changed |= replaceStableColumn(chunk, cursor, geomeIndex, secondGeome, regionalValues, baseRockValue,
							formationRegion, x, z, surfaceY, terrain);
				} else {
					for (int y = surfaceY; y >= chunk.getMinY(); y--) {
						cursor.set(x, y, z);
						BlockState current = chunk.getBlockState(cursor);
						if (terrain.isReplaceable(current) && chunk.getBlockEntity(cursor) == null) {
							StoneReplacer.setRockState(chunk, cursor, current,
									pickReplacement(geomeIndex, baseRockValue, formationRegion, x, y, z));
							changed = true;
						}
					}
				}
	...
	private boolean replaceStableColumn(ChunkAccess chunk, BlockPos.MutableBlockPos cursor, int geomeIndex,
			int secondGeome, double[] geomeScores, int baseRockValue, long formationRegion, int x, int z, int surfaceY,
		...
		for (int y = surfaceY; y >= chunk.getMinY(); y--) {
			...
			BlockState current = chunk.getBlockState(cursor);
			if (terrain.isReplaceable(current) && chunk.getBlockEntity(cursor) == null) {
				BlockState replacement = pickStableReplacement(
						layerGeome, formationRegion, layerIndex, y);
				StoneReplacer.setRockState(chunk, cursor, current, replacement);
				changed = true;
			}
		}
		return changed;
	...
	public ColumnSample sampleColumn(Biome biome, Identifier biomeId, int x, int z) {
		double[] regionalValues = new double[config.geomeCount()];
		int geomeIndex = classifyColumn(biome, biomeId, x, z, regionalValues);
		return new ColumnSample(geomeIndex, runnerUpGeome(regionalValues, geomeIndex), regionalValues,
				stratumOffsetAt(x, z), formationRegionAt(x, z), x, z);
	}
