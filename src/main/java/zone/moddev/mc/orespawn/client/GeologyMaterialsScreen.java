...
final class GeologyMaterialsScreen extends OreSpawnScreen {
	...
	GeologyMaterialsScreen(GuiScreen parent, GeologyEditorSession session) {
		super(new TextComponentTranslation("screen.orespawn.materials"));
		this.parent = parent;
	...
	protected void init() {
		...
		for (int i = 0; i < MaterialTab.values().length; i++) {
			...
			Button button = addButton(OreSpawnScreenLayout.button(this, font,
					...
					new TextComponentTranslation("tab.orespawn." + value.key), selected -> changeTab(value)));
		...
		for (int i = 0; i < pageSize && start + i < ids.size(); i++) {
			...
			ITextComponent rowMessage = new TextComponentString(rowLabel(id));
			...
			if (tab != MaterialTab.UNASSIGNED) {
				addButton(OreSpawnScreenLayout.button(this, font,
						...
						new TextComponentTranslation("button.orespawn.remove"), button -> removeEntry(id)));
			}
		}
	...
	private List<ITextComponent> rowDetails(String id) {
		...
		details.add(new TextComponentString(block));
		if (!block.equals(id)) {
			details.add(new TextComponentTranslation("option.orespawn.registry_id").appendText(": ")
					.appendSibling(new TextComponentString(id)));
		}
		if (tab == MaterialTab.ORES) {
			...
			if (!source.isEmpty()) {
				details.add(new TextComponentString(source));
			}
		}
		return details;
	}
